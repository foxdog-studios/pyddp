...
class SocketTestCase(unittest.TestCase):
    ...
    def test_socket(self):
        opened_cond = threading.Condition()
        def opened_callback(message):
            with opened_cond:
                self.opened = True
                opened_cond.notify()
        ...
        closed_cond = threading.Condition()
        def closed_callback(code, reason=None):
            with closed_cond:
                self.closed = True
                closed_cond.notify()
        ...
        sm = WebSocketClientManager(soc)

        with sm:
            soc.send(MethodMessage(call_id, 'echo', ['Hello, World!']))
            with received_cond:
                while not self.received:
                    received_cond.wait(timeout=1)
        with closed_cond:
            while not self.closed:
                closed_cond.wait(timeout=1)
